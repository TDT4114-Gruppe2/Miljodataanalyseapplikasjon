...
# Legg til `data/raw/` i sys.path for å kunne importere vaerdata.py
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../data/raw")))

import vaerdata  # Nå kan vi importere vaerdata.py

class TestVaerdata(unittest.TestCase):

    @patch("vaerdata.requests.get")  # Mock API-kall
    def test_fetch_weather_data(self, mock_get):
        """Test at API-kallet returnerer forventet JSON-data"""
