...
class YearlyStats(DataLoader):
    ...
    def compute_yearly(
        ...
        *,
        ...
        if aggregate is None:
            ...
            if daily.empty:
                raise ValueError(
                    f"Ingen data for city='{city}', element_id='{element_id}', year={year}"
                )
    ...
    def percent_change(
        ...
        *,
        ...
        if frequency not in {"D", "ME", "YE"}:
            raise ValueError("frequency må være 'D', 'ME' eller 'YE'")
        ...
        df = df[
            (df["elementId"] == element_id) & (df["timeOffset"] == time_offset)
        ].copy()
    ...
    def climatological_monthly_mean(
        ...
        *,
        ...
        clim = agg_map[statistic]().reset_index().rename(columns={"value": "value"})
