...
class MissingWeatherDataAnalyzer:
    ...
    def identify_missing(self):
        oslo_small = self.df_oslo[["date", "timeOffset", "elementId", "value"]].copy()
        oslo_small.rename(columns={"value": "oslo_value"}, inplace=True)

        tromso_small = self.df_tromso[["date", "timeOffset", "elementId", "value"]].copy()
        tromso_small.rename(columns={"value": "tromso_value"}, inplace=True)

        merged = pd.merge(oslo_small, tromso_small, on=["date", "timeOffset", "elementId"], how="outer")
        ...
        self.df_missing = pd.concat([missing_oslo, missing_tromso], ignore_index=True)
